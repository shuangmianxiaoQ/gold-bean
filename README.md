...
- 弹窗每 10 秒、30 秒或 1 分钟刷新，关闭弹窗后每 30 秒后台检查
- 工具栏角标、今日/近 7 日趋势、到价提醒
- 伦敦金与 Au99.99、浙商/民生积存金正反向估算换算
...
Worker 使用 5 秒边缘缓存，并保留 5 分钟最近一次完整结果；单个上游异常时会补入对应的最近成功行情，避免残缺结果覆盖完整缓存。美元兑人民币汇率由 60s 接口提供并单独缓存 15 分钟。
